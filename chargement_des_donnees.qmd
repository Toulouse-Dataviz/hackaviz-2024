...
```{{ojs}}
db = DuckDBClient.of({
  paris_2024: FileAttachment("data/paris_2024.parquet")
})
```
...
pak::pak("geoarrow/geoarrow-r@758a1c1")

library(geoarrow, quietly = TRUE)
library(here, quietly = TRUE)
restaurant <- read_geoparquet_sf(here("data/restaurants_proximité.geoparquet"))
```
...
```{{ojs}}
db = DuckDBClient.of({
  restaurant: FileAttachment("data/restaurants_proximité.geoparquet")
})
```
